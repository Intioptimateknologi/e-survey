...
definePage({
  meta: {
    ...
    description: 'Respondent List page',
    requiresAuth: true,
  }
})
...
const columns: ColumnDef<Respondent>[] = [
  ...
  },
  {
    ...
    header: 'Survey',
    cell: ({ row }) => {
      const survey = row.original.survey
      return (
        <a target="_blank" href={'/s/' + survey?.code} class={`text-start text-blue-500`}>{survey?.title}</a>
      )
    }
  },
  {
...
<template>
  <DashboardLayout>
    <div class="flex justify-between mb-4">
      <div>
        <span class="text-2xl">List Respondent</span>
      </div>
    </div>
